...
sudo -u "$APP_USER" "$APP_DIR/.venv/bin/pip" install -e "$APP_DIR/shared"
sudo -u "$APP_USER" "$APP_DIR/.venv/bin/pip" install -e "$APP_DIR/public-site"

sudo touch "$ENV_FILE"
sudo chmod 600 "$ENV_FILE"

sudo chmod +x "$APP_DIR/public-site/deploy/linux/"*.sh

sudo cp "$APP_DIR/public-site/deploy/systemd/youth-together-pipeline.service" /etc/systemd/system/
sudo cp "$APP_DIR/public-site/deploy/systemd/youth-together-pipeline.timer" /etc/systemd/system/
sudo cp "$APP_DIR/public-site/deploy/systemd/youth-together-backup.service" /etc/systemd/system/
sudo cp "$APP_DIR/public-site/deploy/systemd/youth-together-backup.timer" /etc/systemd/system/
...
do
  ...
  sudo sed -i "s|^WorkingDirectory=.*|WorkingDirectory=$APP_DIR/public-site|" "$service_file"
  sudo sed -i "s|^ExecStart=.*run_pipeline.sh|ExecStart=$APP_DIR/public-site/deploy/linux/run_pipeline.sh|" "$service_file" || true
  sudo sed -i "s|^ExecStart=.*backup_state.sh|ExecStart=$APP_DIR/public-site/deploy/linux/backup_state.sh|" "$service_file" || true
done

sudo cp "$APP_DIR/public-site/deploy/nginx/youth-together.conf" /etc/nginx/sites-available/youth-together.conf
...
echo "  cat $APP_DIR/runtime/pipeline/pipeline_status.json"
