...
이 문서는 청년동향실 저장소의 작업 맥락, 운영 규칙, 재발 방지 지침, 실제 장애 기록을 한 곳에 모아둔 살아 있는 운영 문서다.
...
- 서비스명: `청년동향실`
- 메인 문구: `쏟아지는 청년 이슈를 시간의 흐름으로 읽습니다`
- 운영 주체 표기: `유스사이드(Youthside)`
- `유쾌한청년들` 명칭은 더 이상 사용하지 않는다.

### 1-2. 메뉴 명명과 순서

- 처음 방문한 사람도 내용 유형을 바로 예측할 수 있는 1차 인지 언어를 사용한다.
- 내부 분류어, 은유적 이름, 설명이 필요한 축약어보다 일반적으로 익숙한 콘텐츠 명칭을 우선한다.
- 공개 메뉴 순서는 아래와 같이 고정한다.
  1. 홈 허브
  2. 뉴스 모아보기
  3. 기고·칼럼·오피니언
  4. 정부 부처 자료실
  ...
  7. 정부조사·통계
  8. 분석실
- 상단·사이드 메뉴와 페이지 제목은 같은 명칭을 사용한다. 모바일 하단 메뉴는 공간 제약 때문에 의미가 유지되는 범위에서만 짧게 표기할 수 있다.
...
- 홈 허브는 최근 6시간의 자료량을 30분 단위 `자료량`으로 표시한다. 12개 시간 구간을 유지해 시간축을 왜곡하지 않고, 자료가 없는 구간도 다른 블록과 같은 테두리와 흰색 면으로 표시하되 숫자 `0`은 표시하지 않는다.
- `전체 흐름 보기`는 저장된 과거 6시간 구간을 순차적으로 열고, 이후 스크롤에 맞춰 이전 구간과 해당 자료를 계속 불러온다. 공개 보관 기간 안에서 더 이상 불러올 자료가 없을 때만 종료 상태를 알린다.
- `최신 자료 흐름`에는 뉴스·기고·연구·정부·지자체 자료를 역시간순으로 보여주되, 각 항목의 원래 메뉴를 명시한다.
- 메뉴 분류는 화면 방향 태그보다 자료 형식을 우선한다. `인사이트·분석`으로 판별된 일반 언론기사를 연구자료로 보내지 않으며, 논문·연구·리포트 메뉴는 제목·섹션의 명시적 연구 신호, 연구자료 유형, 신뢰 가능한 연구기관 출처 중 하나를 요구한다.
- 블록의 색 농도는 중요도가 아니라 해당 시간대의 자료 수만 뜻한다.
- `여기까지 읽었습니다` 위치는 이용자의 브라우저에만 저장한다.
- 정치성/선거성 기사 전체를 없애는 것이 아니라, 홈 허브의 즉시성 흐름에서는 보수적으로 다룬다.
